...
Healer::Healer(string name, int health, int attack, double protection, int speed, bool playerCharacter):Hero(name, health, attack, protection, speed, playerCharacter) {

}

void Healer::strike(vector<Hero*>& party) {
    int target = -1;
    int targetHealth;
    int damage;
    while (target == -1) {
        ...
        if (targetHealth == 0) {
            ...
            this->setHealth(this->getHealth() + 0.3 * this->getMaxHealth());
        }
    }
    cout << this->getName() << " dealt "  << damage << " to " << party[target - 1]->getName() << endl;
}

void Healer::ability(vector<Hero*>& party) {
